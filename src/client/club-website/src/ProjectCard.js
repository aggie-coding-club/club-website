  ...
  render(){
    return(
      <div className="projectTile" style={{backgroundColor: "#ffffff", color: "#333e44", margin: 20, width: 400, height: 300}}>
        ...
        <h5 style={{position:"relative", left: 5, fontWeight: 300, margin:0, paddingLeft: 10, paddingRight: 10, paddingTop: 10, paddingBottom: 5, fontSize: "1.25em",textAlign: "left", width: "auto"}}><b>Project Manager:</b> {this.state.projectManager}</h5>
        {this.state.description !== '' ? <p style={{position: "relative", fontWeight: 100, fontSize: "1em", paddingLeft: 15, paddingRight: 15, paddingTop: 5, paddingBottom: 5, textAlign: "left", margin: 0}}>{this.state.description}</p> : null}
        {this.state.members.length > 0 ? <div style={{position: "relative", textAlign: "left", paddingLeft: 15, paddingRight: 15, zIndex: 4, marginTop: 5, width: 370}}><b>Contributors: </b>{this.state.members.map(member=><Chip style={{display: "inline-flex", color: "#ffffff", backgroundColor: "#0288D1", height: 20, margin: 2, verticalAlign: "center"}} key={member.id} onClick={this.handleChipClick(member.avatar_url)} label={member.login}/>)}</div> : null}
        ...
        <p style={{margin: 0, fontSize: "0.8em", position: "absolute", left: 23, bottom: 11.5}}> {this.state.language} </p>
